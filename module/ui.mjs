...
export async function prompt(title,content,label='Apply') {
 return foundry.applications.api.DialogV2.wait({window:{title},classes:['tw'],content:`<div class="tw-dialog">${content}</div>`,buttons:[{action:'apply',label,default:true,callback:(_event,button)=>Object.fromEntries(new FormData(button.form))},{action:'cancel',label:'Cancel',callback:()=>null}]},{rejectClose:false});
}
