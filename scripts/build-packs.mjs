...
// Every field action must carry all four result tiers in its own words, so that a roll
// card and the Actions tab can answer "what happens now?" without the rulebook.
const moves=MOVES.map(([name,stat,reference,category,difficult=false])=>{
 const result=OUTCOMES[name]??[];
 const [success,partial,failure,snakeEyes,doubleSix='']=result;
 for(const [tier,text] of [['10+',success],['7-9',partial],['6 or lower',failure],['snake eyes',snakeEyes]])
  if(!text)throw new Error(`Field action "${name}" has no authored ${tier} result in data/moves.mjs`);
 return item(name,'move',{key:slug(name),stat,category,difficult,combat:['Combat','Titan hunting'].includes(category),source:reference,
  description:html(reference),success,partial,failure,snakeEyes,doubleSix});
});
